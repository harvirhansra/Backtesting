...
from backtesting.trade.pnl import win_or_loss, report_final_pnl
...
class Strategery(object):
    ...
    def __init__(self, market_df, currency, initial_balance=10000, log=True):
        ...
        self.df.loc[:, 'drawdown_pct'] = 0
        self.df.loc[:, 'equity'] = initial_balance
    ...
    def short(self, date, day, quantity=0, max=False):
        ...
        if trade.quantity > 0:
            self.plays.append((trade.date, trade.price, 'short', None))
            self.prev_trade = trade
            if len(self.plays) == 1:
                self.start_btc = trade.quantity
                self.market_entered = True
